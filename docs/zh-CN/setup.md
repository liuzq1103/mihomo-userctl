...
已有管理员公共 Node/npm/Codex 的服务器，先核对[共享运行时规范](shared-runtime.md)。
此流程只准备个人 Mihomo 和控制层，公共程序缺失/不兼容由管理员处理，不自动个人补装。

本页默认从官方在线来源获取软件。课题组已提前备包时，请先阅读
[共享目录本地安装与下载清单](offline-install.md)，用该流程替代下文的下载/克隆步骤。
