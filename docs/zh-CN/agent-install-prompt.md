...
只有 proxy_on 或 with_proxy 显式启用代理。只操作我的账户，不影响其他用户。
...
1. 在做其他事情前，先确认你具备以下能力：
   ...
   只给出缺失部分的准确人工操作说明。
2. 只进行只读审计：确认当前用户、HOME、CPU 架构、Linux 发行版、Bash 版本、
   systemd 用户管理器、所需命令、已有 Mihomo 文件和用户服务、Shell 启动文件、
   当前监听、候选项目检出目录，以及可能修改的文件。本阶段不得使用 sudo、
   root、其他账户、系统级服务、linger、cron、TUN、controller 或系统代理；
   不得停止进程、变更端口、启动服务或写文件。归属不明的监听和其他用户完全
   不在范围内。
3. 通过普通对话向我逐项确认非敏感选项，每次最多提出三个简短问题。确认项目
   检出路径、Shell 启动文件、readiness URL 和最终的每用户端口。项目没有默认
   端口；同一服务器上的用户必须选择不同端口。不得要求我把敏感信息粘贴到聊天。
...
9. 从 docs/zh-CN/setup.md 的 Mihomo 安装步骤开始执行。Mihomo 只能从官方 Release
   ...
   MATCH,DIRECT。
10. 非敏感 Shell 设置写入 mihomo-shell.conf；带认证的本地端点 URL 写入权限
    600 的 client.env。订阅 URL、凭据、provider 内容和私有节点数据不得进入
    Git、聊天、命令参数、日志、截图、diff 或报告。
11. 在已验证的检出目录中执行确定性控制层流程：先运行
    ./install.sh --dry-run --port PORT，再运行 ./install.sh --port PORT --bashrc
    PATH。只能替换阶段一确认过的值。不得 enable 服务，不得修改当前用户获批
    路径以外的文件。
12. 启动服务前先验证配置语法。测试启动只能使用 systemctl --user。确认
    systemctl --user is-enabled mihomo 始终返回 disabled。不得启用 linger，
    也不得创建任何自动启动机制。
13. 在不暴露敏感信息的前提下完成最终验收：认证 HTTP 和 SOCKS5H 成功；未认证
    请求失败；Listener 只监听所选端口的 127.0.0.1；新 Shell 默认直连；
    proxy_on 和 proxy_off 正常；with_proxy 只影响子 Shell；普通 axel、S3 和
    大文件下载保持直连；停止服务后端口释放；日志和项目检出中没有敏感值；
    其他用户与无关监听没有发生变化。
14. 如果写入或验收失败，立即停止；在安全且可验证时从备份恢复受影响文件，
    并报告剩余状态。绝不杀死未知进程、扩大权限、使用 sudo 或悄悄弱化检查。
15. 最后提供脱敏报告：版本和已验证摘要、所选端口、修改文件、备份位置、服务
