...
订阅 URL 或密码粘贴进 Prompt、弹窗或聊天；秘密只应进入服务器权限 `600` 的
本地文件。
...
3. 运行 ./install.sh --suggest-port 取得候选端口；同时检查当前监听。通过交互
   弹窗让我确认候选值或填写自定义端口。同一服务器用户必须避开彼此端口；
   建议结果不是预留，正式绑定前必须再次检查。
4. 使用交互弹窗收集其他非秘密选择，给出完整 Plan 和精确修改范围；任何步骤
   需要管理员权限，或现有配置归属不明时立即停止。
5. 修改前对范围内文件创建权限收紧的备份。
6. 从 MetaCubeX 官方 Release 安装固定版本 Mihomo：下载匹配资产，核对官方
   SHA256，验证候选版本和配置后再原子安装到 ~/.local/bin。
7. 创建仅回环、必须认证的配置、私有 provider/cache、用户 systemd unit 和
   权限 600 的 client.env，全程不显示秘密。
8. 克隆或使用 mihomo-userctl，先运行测试，再把已确认端口显式传给
   install.sh --dry-run --port 和 install.sh --port --bashrc "$HOME/.bashrc"。
9. 不 enable、不设置自动启动；仅为测试手动启动服务。
10. 验证：未认证 HTTP/SOCKS 被拒绝；认证 HTTP/SOCKS5H 成功；服务 disabled；
