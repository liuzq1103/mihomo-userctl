...
MIHOMO_HTTP_PROXY='http://user:0123456789abcdef@127.0.0.1:17890'
MIHOMO_HTTPS_PROXY='http://user:0123456789abcdef@127.0.0.1:17890'
MIHOMO_ALL_PROXY='socks5h://user:0123456789abcdef@127.0.0.1:17890'
EOF
...
printf 'LISTEN 0 4096 127.0.0.1:17890 0.0.0.0:*\\n'
EOF
...
assert 'status reports active loopback listener' bash -c '[[ $(mihomoctl status) == *"service=up enabled=disabled listener=up endpoint=127.0.0.1:17890"* ]]'
...
assert 'shell module starts direct and proxy_on exports eight variables' bash -c '
  ...
  [[ -n $http_proxy && -n $https_proxy && -n $all_proxy && -n $no_proxy &&
     -n $HTTP_PROXY && -n $HTTPS_PROXY && -n $ALL_PROXY && -n $NO_PROXY ]]
  [[ $(proxy_status) == "shell=proxied service=up endpoint=127.0.0.1:17890" ]]
'
...
# Installer migration and idempotency use a second temporary HOME.
...
assert 'installer is idempotent' env HOME="$INSTALL_HOME" XDG_CONFIG_HOME="$INSTALL_HOME/.config" XDG_DATA_HOME="$INSTALL_HOME/.local/share" PATH="$PATH" bash "$ROOT/install.sh" --port 17890 --bashrc "$INSTALL_HOME/.bashrc"
